create table public.ai_proposals (
  ...
  kind        text not null check (
    kind in ('set_nutrition_day_type', 'modify_session', 'skip_session', 'add_todo', 'reorder_todos')
  ),
...
create index ai_proposals_brief_status_idx
  on public.ai_proposals (brief_id, status);
...
create policy "owner_select_ai_proposals" on public.ai_proposals
  for select to authenticated using (is_owner());
create policy "owner_insert_ai_proposals" on public.ai_proposals
  for insert to authenticated with check (is_owner());
create policy "owner_update_ai_proposals" on public.ai_proposals
